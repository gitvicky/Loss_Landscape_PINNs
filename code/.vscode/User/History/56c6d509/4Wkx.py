...
path = os.getcwd()
results = os.path.dirname(os.path.dirname(os.path.dirname(os.getcwd())))
...
def Burgers_PINN(sparse_val):
    ...
    torch.save({
        ...
        'loss': loss
    }, path + results + '/Burgers_'+str(run_id)+'.pth')
...
for ii in range(len(models)):
    ...
    # Checkpoint
    model_loc = path + results + '/'+ models[ii]
    ...
    ax.set_title('Logarithm: Surface Plot of Loss Landscape for ' + name)
    plt.savefig(path + '/Results/loss_landscape_'+name+'_'+'.png')
    

    np.save(path + '/Results/Loss_Surface_' + name, losses)
